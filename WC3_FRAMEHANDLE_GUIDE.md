...
- **Animated autocast border (`autocastBorder`)** uses a `SPRITE` with `UI\\Feedback\\Autocast\\UI-ModalButtonOn.mdl`. SPRITEs loop automatically and are non-interactive, but changing their parent at runtime can permanently speed up the animation ([Tasyen's frame tutorial](https://www.hiveworkshop.com/threads/the-big-ui-frame-tutorial.335296/)). The safe lifecycle is one effect created under each final target, permanently anchored there, then shown/hidden as state changes. Do not move one shared effect between targets with `setParent` or post-creation reparenting. `UIAutocastBorder` enforces this by exposing visibility only, requires an explicit target size for model scale, and never destroys the WC3 frame.
...
- `autocastBorder(target, size)` adds the animated native autocast border without exposing unsafe SPRITE reparenting.
